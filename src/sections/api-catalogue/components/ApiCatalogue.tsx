  ...
  return (
    <div className="min-h-screen bg-stone-100 dark:bg-stone-950">
      ...
      <div className="flex">
        ...
        {/* Main content area */}
        <div className="flex-1 min-w-0 px-5 md:pl-0 sm:pr-6 lg:pr-8 pb-5 sm:pb-6 lg:pb-8 pt-0">
          {/* Mobile Tab Switcher */}
          {!selectedApi && (
            <div className="flex md:hidden items-center gap-1 p-1 bg-stone-200/40 dark:bg-stone-900 rounded-lg w-full mb-5">
              {SIDEBAR_ITEMS.map((item) => {
                const Icon = item.icon
                ...
                return (
                  <button
                    key={item.id}
                    onClick={() => handleSidebarTab(item.id)}
                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 min-h-11 rounded-md text-sm font-medium transition-colors ${
                      isMobileActive
